...
def lstm(rnn_size, keep_prob, reuse=False):
    lstm_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(rnn_size, reuse=reuse)
...
def model_input():
    ...
    target_data = tf.compat.v1.placeholder(tf.int32, [None, None], name='target')
    input_data_len = tf.compat.v1.placeholder(tf.int32, [None], name='input_len')
    target_data_len = tf.compat.v1.placeholder(tf.int32, [None], name='target_len')
    lr_rate = tf.compat.v1.placeholder(tf.float32, name='lr')
...
def encoder_input(source_vocab_size, embed_size, input_data):
    encoder_embeddings = tf.Variable(tf.random.uniform([source_vocab_size, embed_size], -1, 1))
...
def encoder_layer(stacked_cells, encoder_embedded, input_data_len):
    ((encoder_fw_outputs, encoder_bw_outputs),
     (encoder_fw_final_state, encoder_bw_final_state)) = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell_fw=stacked_cells,
    ...
    encoder_states = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(c=encoder_state_c, h=encoder_state_h)

    return encoder_outputs, encoder_states


def attention_layer(rnn_size, encoder_outputs, dec_cell, target_data_len, batch_size, encoder_states):
    attention_mechanism = tfa.seq2seq.BahdanauAttention(rnn_size * 2, encoder_outputs,
                                                               memory_sequence_length=target_data_len)

    attention_cell = tfa.seq2seq.AttentionWrapper(dec_cell, attention_mechanism,
...
def decoder_embedding(target_vocab_size, embed_size, decoder_input):
    decoder_embeddings = tf.Variable(tf.random.uniform([target_vocab_size, embed_size], -1, 1))
...
def decoder_train_layer(rnn_size, decoder_input,
    ...
    train_helper = tfa.seq2seq.TrainingSampler(dec_cell_inputs, target_data_len) # TODO ver bem esta linha

    decoder_train = tfa.seq2seq.BasicDecoder(cell=attention_cell, helper=train_helper,
                                                    initial_state=state,
                                                    output_layer=dense_layer)

    outputs_train, _, _ = tfa.seq2seq.dynamic_decode(decoder_train,
...
def decoder_infer_layer(decoder_embeddings, batch_size, vocabs_to_index,
    ...
    infer_helper = tfa.seq2seq.GreedyEmbeddingSampler(decoder_embeddings,
                                                            tf.fill([batch_size], vocabs_to_index['<GO>']),
                                                            vocabs_to_index['<EOS>'])  # TODO ver bem esta linha

    decoder_infer = tfa.seq2seq.BasicDecoder(cell=attention_cell, helper=infer_helper,
                                                    initial_state=state,
                                                    output_layer=dense_layer)

    outputs_infer, _, _ = tfa.seq2seq.dynamic_decode(decoder_infer, impute_finished=True,
                                                            maximum_iterations=tf.reduce_max(target_data_len))
...
def opt_loss(outputs_train, outputs_infer, target_data_len, target_data, lr_rate):
    ...
    cost = tfa.seq2seq.sequence_loss(training_logits, target_data, masks)
    optimizer = tf.compat.v1.train.AdamOptimizer(lr_rate)
...
def seq2seq_model(source_vocab_size, embed_size, rnn_size, keep_prob,
    ...
    dense_layer = tf.keras.layers.Dense(target_vocab_size)
